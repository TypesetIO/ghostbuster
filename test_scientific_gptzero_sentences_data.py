...
def predict_text(text):
    ...
    model_directory = "paragraph_level_model/"
...
files = glob(path)[:]
files += glob("/home/asheesh/Documents/beast/ai-detector-survey/generate_data/data_anthropic/*.json")
# files = files[:640]+files[650:]
predictions = []
predictions_sentences_gptzero = []
ground_truth = []
for file in tqdm.tqdm(files):
    ...
    if 'original_gptzero_result_sentences' in data.keys():
        result = data['original_gptzero_result_sentences']
        sentences = result['documents'][0]['sentences']
        for sentence in sentences:
            prediction = predict_text(sentence['sentence'])
            predictions.append(prediction)
            if not sentence['highlight_sentence_for_ai']:
                predictions_sentences_gptzero.append(0)
            else:
                predictions_sentences_gptzero.append(1)
            ground_truth.append(0)
    if 'generated_gptzero_result_sentences' in data.keys():
        result = data['generated_gptzero_result_sentences']
        sentences = result['documents'][0]['sentences']
        for sentence in sentences:
            prediction = predict_text(sentence['sentence'])
            predictions.append(prediction)
...
# Calculate metrics for our model's predictions
print("\n=== Our Model Results ===")
conf_matrix = confusion_matrix(ground_truth, predictions)
conf_matrix_normalized = conf_matrix.astype(
    'float') / conf_matrix.sum(axis=1)[:, np.newaxis]

# # Create heatmap for our model
# plt.figure(figsize=(10, 7))
# sns.heatmap(conf_matrix_normalized, annot=True, fmt=".2f", cmap="Blues",
#             xticklabels=['Human', 'GPT'], yticklabels=['Human', 'GPT'])
# plt.xlabel('Predicted')
# plt.ylabel('True')
# plt.title('Normalized Confusion Matrix - Our Model')
# plt.savefig('normalized_confusion_matrix_our_model.png')
# plt.close()
...
# Calculate metrics for GPTZero's predictions
print("\n=== GPTZero Results ===")
conf_matrix_gptzero = confusion_matrix(
    ground_truth, predictions_sentences_gptzero)
conf_matrix_normalized_gptzero = conf_matrix_gptzero.astype(
    'float') / conf_matrix_gptzero.sum(axis=1)[:, np.newaxis]

# # Create heatmap for GPTZero
# plt.figure(figsize=(10, 7))
# sns.heatmap(conf_matrix_normalized_gptzero, annot=True, fmt=".2f", cmap="Blues",
#             xticklabels=['Human', 'GPT'], yticklabels=['Human', 'GPT'])
# plt.xlabel('Predicted')
# plt.ylabel('True')
# plt.title('Normalized Confusion Matrix - GPTZero')
# plt.savefig('normalized_confusion_matrix_gptzero.png')
# plt.close()

print("\nConfusion Matrix (GPTZero):")
print(conf_matrix_gptzero)

print("\nClassification Report (GPTZero):")
print(classification_report(ground_truth,
      predictions_sentences_gptzero, target_names=['Human', 'GPT']))

accuracy_gptzero = accuracy_score(ground_truth, predictions_sentences_gptzero)
print(f"Accuracy (GPTZero): {accuracy_gptzero:.4f}")

# Compare the models
print("\n=== Model Comparison ===")
print(f"Our Model Accuracy: {accuracy:.4f}")
print(f"GPTZero Accuracy: {accuracy_gptzero:.4f}")
print(
    f"Accuracy Difference (Our Model - GPTZero): {accuracy - accuracy_gptzero:.4f}")
