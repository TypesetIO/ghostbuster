...
def predict_text(text):
    ...
    path = 'quantized_paragraph_level_model'
    with open(os.path.join(path, "llama_model.pkl"), "rb") as f:
        logistic_model = pickle.load(f)
    mu = pickle.load(open(os.path.join(path, "mu"), "rb"))
    sigma = pickle.load(open(os.path.join(path, "sigma"), "rb"))
...
for file in tqdm.tqdm(files):
    ...
    if not original_paragraph or not generated_paragraph:
        continue
    prediction = predict_text(original_paragraph)
    # prediction for original paragraph
    predictions.append(prediction)
    # ground truth for original paragraph
    ground_truth.append(0)

    # prediction for generated paragraph
    prediction_direct = predict_text(generated_paragraph)
    predictions.append(prediction_direct)
    # ground truth for generated paragraph
...
plt.savefig('normalized_confusion_matrix_gpt.png')
plt.close()
